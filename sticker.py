...
import uuid
def handle(revimage):
    file_id = revimage.message['sticker']['file_id']
    set_name = revimage.message['sticker'].get('set_name', 'NoSetName')
    logging.info("[%s] send a sticker from %s." % (revimage.sender, set_name))
    ...
    if r.status_code == requests.codes.ok and r.json()['ok']:
        ...
        with open(filename + '.png', 'rb') as png:
            ...
            r = requests.post(url, {"chat_id": revimage.chat_id}, files=files)
